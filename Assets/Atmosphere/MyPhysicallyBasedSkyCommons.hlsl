#ifndef MY_PHYSICALLY_BASED_SKY_DEFINES_HLSL
#define MY_PHYSICALLY_BASED_SKY_DEFINES_HLSL
...
)
{
    ...
    return (
               ...
               light_intensity +
           scene_color * opacity; // now make sure the background is rendered correctly
}
...
float4 render_scene(float3 pos, float3 dir, float3 light_dir)
{
    ...
    if (0.0 < planet_intersect.y)
    {
        ...
        // apply skylight
        color.xyz += clamp(skylight(sample_pos, surface_normal, light_dir, 0.0) * float3(0.0, 0.25, 0.05), 0.0, 1.0);
    }

    return color;
}

// For ShaderGraph Usage

void CalculateScattering_float(
    float3 start,             // the start of the ray (the camera position)
    float3 dir,               // the direction of the ray (the camera vector)
    float max_dist,           // the maximum distance the ray can travel (because something is in the way, like an object)
    float3 scene_color,       // the color of the scene
    ...
    out float3 scattering)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // calculate and return the final color
            scattering = (phase_ray * beta_ray * total_ray   // rayleigh color
                          + phase_mie * beta_mie * total_mie // mie
                          + opt_i.x * beta_ambient           // and ambient
                          ) *
                             light_intensity +
    ...
    out float2 inter)
{
    ...
    float d = (b * b) - 4.0 * a * c;
    if (d < 0.0)
        inter = float2(1e5, -1e5);
    else
        inter = float2((-b - sqrt(d)) / (2.0 * a), (-b + sqrt(d)) / (2.0 * a));
}

void SkyLight_float(
    float3 cameraPos, 
    float3 cameraPosNormal, 
    float3 lightDir, 
...
{
    // and sample the atmosphere
    skyLight = calculate_scattering(
        cameraPos,               // the position of the camera
        cameraPosNormal,         // the camera vector (ray direction of this pixel)
        3.0 * atmosphereRadius,  // max dist, since nothing will stop the ray here, just use some arbitrary value
        backgroundColor,         // scene color, just the background color here
        lightDir,                // light direction
        float3(40.0, 40.0, 40.0),// light intensity, 40 looks nice
        planetPos,               // position of the planet
        planetRadius,            // radius of the planet in meters
        atmosphereRadius,        // radius of the atmosphere in meters
        betaRayLeigh,            // Rayleigh scattering coefficient
        betaMie,                 // Mie scattering coefficient
        betaAbsorption,          // absorption coefficient
        betaAmbient,             // ambient scattering, turned off for now. This causes the air to glow a bit when no light reaches it
        g,                       // Mie preferred scattering direction
        heightRayLeigh,          // Rayleigh scale height
        heightMie,               // Mie scale height
        heightAbsorption,        // the height at which the most absorption happens
        absorptionFalloff,       // how fast the absorption falls off from the absorption height
        lightSteps,              // steps in the ray direction
...
*/
void RenderScene_float(float3 viewPositionWS, float3 ViewDirection, float3 MainLightDirection, float3 planetPos, float planetRadius, out float4 SceneColor)
{
    // the color to use, w is the scene depth
    SceneColor = float4(0.0, 0.0, 0.0, 1e12);

    // add a sun, if the angle between the ray direction and the light direction is small enough, color the pixels white
    SceneColor.xyz = dot(ViewDirection, MainLightDirection) > 0.9998 ? 3.0 : 0.0;

    // get where the ray intersects the planet
    float2 planet_intersect = ray_sphere_intersect(viewPositionWS - planetPos, MainLightDirection, planetRadius);

    // if the ray hit the planet, set the max distance to that ray
    if (0.0 < planet_intersect.y)
    {
        SceneColor.w = max(planet_intersect.x, 0.0);

        // sample position, where the pixel is
        float3 sample_pos = viewPositionWS + (MainLightDirection * planet_intersect.x) - planetPos;

        // and the surface normal
        float3 surface_normal = normalize(sample_pos);

        // get the color of the sphere
        SceneColor.xyz = float3(0.0, 0.25, 0.05);
        ...
        // apply the shadow
        SceneColor.xyz *= shadow;

        // apply skylight
        surface_normal = normalize(lerp(surface_normal, MainLightDirection, 0.6));

        float3 skyLight = calculate_scattering(
            sample_pos,               // the position of the camera
            surface_normal,           // the camera vector (ray direction of this pixel)
            3.0 * ATMOS_RADIUS,       // max dist, since nothing will stop the ray here, just use some arbitrary value
            0,                        // scene color, just the background color here
            MainLightDirection,       // light direction
            float3(40.0, 40.0, 40.0), // light intensity, 40 looks nice
            planetPos,                // position of the planet
            planetRadius,             // radius of the planet in meters
            ATMOS_RADIUS,             // radius of the atmosphere in meters
        ...
        SceneColor.xyz += clamp(skyLight * float3(0.0, 0.25, 0.05), 0.0, 1.0);
    }
}

#endif // MY_PHYSICALLY_BASED_SKY_DEFINES_HLSL