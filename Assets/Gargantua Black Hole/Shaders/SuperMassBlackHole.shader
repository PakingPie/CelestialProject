    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float4 frag (v2f IN) : SV_Target
			{
				...
				float3 texCol = _DiscTex.SampleLevel(sampler_DiscTex, uv * _DiscTex_ST.xy, 0).rgb;
				...
				transmittance *= _DiscColor.a;
				float3 col = lerp(backgroundCol, discCol * texCol, transmittance);
				return float4(col,1);
			}
            ENDHLSL
        }
