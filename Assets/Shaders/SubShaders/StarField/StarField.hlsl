#ifndef CUSTOM_STARFIELD_HLSL
#define CUSTOM_STARFIELD_HLSL
#define ANIMATE
#define OCTAVES 5
...
void HSV2RGB_float(float3 c, out float3 rgb)
{
...
float2 mod(float2 x, float y)
{
    ...
    return x - abs(y * floor(x / y));
}

// Thanks to David Hoskins https://www.shadertoy.com/view/4djGRh
void Stars_float(float2 x, float numCells, float size, float br, out float stars)
{
    float2 n = x * numCells;
    float2 f = floor(n);
    ...
    stars = br * (smoothstep(.95, 1., (1. - sqrt(d))));
}
...
void FractalNebula_float(float2 coord, float3 color, float transparency, out float3 nebula)
{
    float n = fractalNoise(coord, 0.5, 2.0);
    nebula = n * color * transparency;
}
